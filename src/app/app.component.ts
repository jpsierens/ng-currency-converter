...
export class AppComponent implements OnInit {
    ...
    constructor(public store: Store<fromRoot.State>) {
        // store the result of the selectors(queries) to the state in variables.
        this.amount$ = store.select(fromRoot.getAmountState);
        this.currencyRates$ = store.select(fromRoot.getCurrencyRates);
    }

    // Dispatch the Action to get the currencies when the app starts
    ngOnInit() {
        this.store.dispatch(new CurrenciesUpdateAction());
    }

    // callback to when the input is changed
